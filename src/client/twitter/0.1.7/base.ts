...
export class ClientBase extends EventEmitter {
    ...
    async init() {
        ...
        while (retries > 0) {
            await new Promise(resolve => setTimeout(resolve, 10e3)); // wait for 10s before retries

            try {
                if (await this.twitterClient.isLoggedIn()) { // cookies are valid, no login required
                    elizaLogger.info('Successfully logged in.');
                    tokimonsterNotification.info('[Tokimonster][TwitterClient] Successfully logged in with cached cookies');
                    break;
                } else {
                    ...
                    if (await this.twitterClient.isLoggedIn()) {  // fresh login, store new cookies
                        elizaLogger.info('Successfully logged in.');
                        tokimonsterNotification.info('[Tokimonster][TwitterClient] Successfully logged in');
            ...
            if (retries === 0) {
                ...
                tokimonsterNotification.error('[Tokimonster][TwitterClient] Max retries reached. Exiting login process');
